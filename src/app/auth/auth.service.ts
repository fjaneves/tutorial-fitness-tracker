...
export class AuthService {
    ...
    registerUser(authData: AuthData){
        ...
        this.afAuth.auth.createUserWithEmailAndPassword(authData.email, authData.password).then(
            result => {
                ...
                this.uiService.showSnackbarSuccess("Registro realizado com sucesso!");
            }
        ).catch(error => {
            this.uiService.loadingStateChanges.next(false);
            this.uiService.showSnackbarError(error.message);
        })
    }
    ...
    login(authData: AuthData) {
        ...
        this.afAuth.auth.signInWithEmailAndPassword(authData.email, authData.password).then(
            result => {
                this.uiService.loadingStateChanges.next(false);
                this.uiService.showSnackbarSuccess("Login realizado com sucesso!");
            }
        ).catch(error => {
            this.uiService.loadingStateChanges.next(false);
            this.uiService.showSnackbarError(error.message);
